...
@click.option(
    "-f", "--file", default=None, help="The destination output file"
)
@click.option(
    "-v",
    "--verbosity",
    ...
    async with AIOFile(dest_path, "w") as aiodf:
        ...
        for thing in thing_iterator.yieldRecordsByPage():
            ...
            if counter % 1000 == 0:
                ...
                print(f"{current_time - start} seconds elapsed, {counter} records processed")
            try:
                ...
                material_category_confidences = transformer.has_material_category_confidences(material_categories)
                material_categories_str = ",".join(material_categories)
                if material_category_confidences is not None and len(material_category_confidences) > 0:
                    material_category_confidences_str = ",".join([str(confidence) for confidence in material_category_confidences])
                else:
